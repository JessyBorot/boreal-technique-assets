...
function boot() {
  ...
  CustomEase.create("radial", "0.25, 0.1, 0, 1"); // slider radial (Osmo) — section « types d'événements »
...
// Structure Webflow : [data-radial-slider-init] > [data-radial-slider-collection] > [data-radial-slider-list]
//   > [data-radial-slider-item] × N. Contrôles [data-radial-slider-control="prev|next|<n>"] (dots/compteur optionnels).
// GSAP + Draggable + InertiaPlugin + CustomEase déjà chargés/registrés (head) ; ease "radial" créée dans boot().
// Géométrie pilotée par le CSS (--slider-rotate / --slider-radius) — voir boreal-styles.css.
// Suivi global → destroy des instances de la page précédente aux transitions Barba (kill Draggable + retire le proxy).
let _radialSliders = [];
let _radialResizeHandler = null;
function initRadialCardsSlider() {
  // Nettoie les instances précédentes (Barba / resize) : sinon des Draggables orphelins gardent leurs listeners.
  _radialSliders.forEach((s) => { try { s.destroy(); } catch (_) {} });
  _radialSliders = [];
  ...
  document.querySelectorAll("[data-radial-slider-init]").forEach((container) => {
    ...
    container.setAttribute("aria-label", container.getAttribute("aria-label") || "Types d'événements");
    ...
    controls.forEach((btn) => {
      ...
      if (value === "next" || value === "prev") {
        btn.onclick = () => {
          ...
          gsap.to(proxy, { rotation: -targetIndex * rotateStep, duration: slideDuration, ease: clickEase, onUpdate: render });
        };
      }
      if (/^\d+$/.test(value)) {
        ...
        btn.onclick = () => {
          ...
          gsap.to(proxy, { rotation: -(currentIndex + delta) * rotateStep, duration: slideDuration, ease: clickEase, onUpdate: render });
        };
      }
    });
    ...
    _radialSliders.push({
      destroy() {
        try { container._radialSliderDraggable && container._radialSliderDraggable.kill(); } catch (_) {}
        try { gsap.killTweensOf(proxy); } catch (_) {}
        try { proxyWrap.remove(); } catch (_) {}
        container.querySelectorAll("[data-radial-slider-clone]").forEach((el) => el.remove());
        controls.forEach((btn) => { btn.onclick = null; });
  ...
  // resize : recalcul complet (comportement Osmo). Réattaché seulement s'il y a au moins un slider sur la page.
  if (_radialResizeHandler) { window.removeEventListener("resize", _radialResizeHandler); _radialResizeHandler = null; }
  if (_radialSliders.length) {
...
// ---- BUNNY HLS PLAYER (Osmo advanced) — lecteur vidéo HLS, page Réalisation (T07) ----
// [data-bunny-player-init] + data-player-src="<url .m3u8>". Autoplay : data-player-autoplay="true"
// (force muted + loop + IntersectionObserver play/pause selon la visibilité). Contrôles délégués
// [data-player-control="playpause|mute|fullscreen"]. Requiert hls.js (chargé dans le footer, avant ce fichier).
// Barba-safe : registre _bunnyPlayers → destroy (hls, IntersectionObserver, rAF, listeners globaux) à chaque ré-init.
let _bunnyPlayers = [];
function initBunnyPlayer() {
  _bunnyPlayers.forEach(function (p) { try { p.destroy(); } catch (_) {} });
  _bunnyPlayers = [];
  ...
  document.querySelectorAll("[data-bunny-player-init]").forEach(function (player) {
    ...
    // Listeners globaux (document/window) enregistrés pour retrait à la destruction (Barba).
    var off = [];
    function on(target, type, fn, opts) { target.addEventListener(type, fn, opts); off.push(function () { target.removeEventListener(type, fn, opts); }); }
    var io = null, rafId = null, trackingMove = false, onPointerMoveGlobal = null;
    ...
    function fetchMetaOnce() {
      getSourceMeta(src, canUseHlsJs).then(function (meta) {
        ...
        if (timeDurationEls.length && isFinite(meta.duration) && meta.duration > 0) setText(timeDurationEls, formatTime(meta.duration));
    ...
    function attachMediaOnce() {
      ...
      if (isSafariNative) {
        ...
        hls.on(Hls.Events.MANIFEST_PARSED, function () {
          readyIfIdle(player, pendingPlay);
          if (updateSize === "true") { var best = bestLevel(hls.levels || []); if (best && best.width && best.height) setBeforeRatio(player, updateSize, best.width, best.height); }
        });
        hls.on(Hls.Events.LEVEL_LOADED, function (e, data) {
          if (data && data.details && isFinite(data.details.totalduration) && timeDurationEls.length) setText(timeDurationEls, formatTime(data.details.totalduration));
        });
        player._hls = hls;
      } else {
        video.src = src;
    ...
    if (isLazyMeta) { fetchMetaOnce(); video.preload = "none"; }
    else if (isLazyTrue) { video.preload = "none"; }
    else { attachMediaOnce(); }

    function togglePlay() {
      if (video.paused || video.ended) {
        if ((isLazyTrue || isLazyMeta) && !isAttached) attachMediaOnce();
        pendingPlay = true; setStatus("loading"); safePlay(video);
      } else { video.pause(); }
    }
    function toggleMute() { video.muted = !video.muted; player.setAttribute("data-player-muted", video.muted ? "true" : "false"); }
    ...
    var onFsChange = function () { setFsAttr(isFsActive()); };
    ...
    if (timeline) {
      ...
      var getFractionFromX = function (x) { if (!rect) rect = timeline.getBoundingClientRect(); var f = (x - rect.left) / rect.width; if (f < 0) f = 0; if (f > 1) f = 1; return f; };
      var previewAtFraction = function (f) {
        if (!video.duration) return;
        var pct = f * 100;
      ...
      var maybeSeek = function (now) { if (!video.duration) return; if ((now - lastSeekTs) < seekThrottle) return; lastSeekTs = now; video.currentTime = targetTime; };
      var onPointerMove = function (e) { if (!dragging) return; var f = getFractionFromX(e.clientX); targetTime = f * video.duration; previewAtFraction(f); maybeSeek(performance.now()); e.preventDefault(); };
    ...
    on(document, "fullscreenchange", wakeControls);
    on(document, "webkitfullscreenchange", wakeControls);
    onPointerMoveGlobal = function (e) {
    ...
    if (autoplay) {
      io = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          ...
          if (inView) {
            if ((isLazyTrue || isLazyMeta) && !isAttached) attachMediaOnce();
            if (video.paused) { pendingPlay = true; setStatus("loading"); safePlay(video); } else { setStatus("playing"); }
          } else if (!video.paused && !video.ended) { video.pause(); setStatus("paused"); }
        });
      }, { threshold: 0.1 });
      io.observe(player);
    }
    ...
    _bunnyPlayers.push({
      destroy: function () {
        try { off.forEach(function (fn) { fn(); }); } catch (_) {}
        if (io) { try { io.disconnect(); } catch (_) {} }
        if (rafId) cancelAnimationFrame(rafId);
        if (trackingMove && onPointerMoveGlobal) { try { window.removeEventListener("pointermove", onPointerMoveGlobal); } catch (_) {} }
        if (player._hls) { try { player._hls.destroy(); } catch (_) {} player._hls = null; }
        try { video.pause(); } catch (_) {}
      }
    });
  ...
  function bestLevel(levels) { if (!levels || !levels.length) return null; return levels.reduce(function (a, b) { return ((b.width || 0) > (a.width || 0)) ? b : a; }, levels[0]); }
  function safePlay(v) { var p = v.play(); if (p && typeof p.then === "function") p.catch(function () {}); }
  function readyIfIdle(player, pendingPlay) {
    if (!pendingPlay && player.getAttribute("data-player-activated") !== "true" && player.getAttribute("data-player-status") === "idle") player.setAttribute("data-player-status", "ready");
  }
  ...
  function maybeSetRatioFromVideo(player, updateSize, video) {
    ...
    if (!hasPad && video.videoWidth && video.videoHeight) setBeforeRatio(player, updateSize, video.videoWidth, video.videoHeight);
  }
      ...
      if (useHlsJs && window.Hls && Hls.isSupported()) {
        try {
          ...
          tmp.on(Hls.Events.LEVEL_LOADED, function (e, data) {
            ...
            try { tmp.destroy(); } catch (_) {} resolve(out);
          });
      ...
      function parseMaster(masterText) {
        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          if (line.indexOf("#EXT-X-STREAM-INF:") === 0) lastInf = line;
            ...
            var mm = /RESOLUTION=(\d+)x(\d+)/.exec(lastInf);
            if (mm) { var w = parseInt(mm[1], 10), h = parseInt(mm[2], 10); if (w > bestW) { bestW = w; bestH = h; } }
            lastInf = null;
          }
        }
        return { bestW: bestW, bestH: bestH, media: firstMedia };
      }
      function sumDuration(mediaText) { var dur = 0, re = /#EXTINF:([\d.]+)/g, mm; while ((mm = re.exec(mediaText))) dur += parseFloat(mm[1]); return dur; }
      fetch(src, { credentials: "omit", cache: "no-store" }).then(function (r) { if (!r.ok) throw new Error("master"); return r.text(); })
        .then(function (master) {
          ...
          return fetch(mediaUrl, { credentials: "omit", cache: "no-store" }).then(function (r) { if (!r.ok) throw new Error("media"); return r.text(); })
            .then(function (mediaText) { resolve({ width: info.bestW || 0, height: info.bestH || 0, duration: sumDuration(mediaText) }); });
